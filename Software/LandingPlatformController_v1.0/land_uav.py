...
while True:
    ...
    move_uav(direction_vec=target_move_vec,speed=0.3) #inch forward in x axis
    print('moving')
    # time.sleep(1)
    coords = get_coords() #get new coords after that is done
    print(coords)
    if get_distance(coords,first_coords) < 10:
        print('didnt move')
        continue
    if coords[0] == 900: #if we left the frame, come back
        print('left frame')
        move_uav(direction_vec=-target_move_vec,speed=0.4) #inch forward in x
        # time.sleep(1)
        coords = get_coords()
        print(coords)
    ...
    if np.allclose(coords,target_coords,atol=12):
