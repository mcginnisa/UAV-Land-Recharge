...
pink_threshold = [(0, 100, 54, 127, -5, 48)]
green_threshold = [(13, 49, -128, -21, -11, 47)]
green_led = [(0, 90, -128, 13, 19, 80)]
green_tape_100 =[(36, 81, -128, -16, -14, 48)]

green_tape_1 = [(0, 32, -128, -10, 10, 127)]
pink_tape_1 = [(0, 57, 5, 127, 16, 127)]
...
def getthoseblobs(img):
    ...
    for blob in img.find_blobs(green_tape_1, pixels_threshold=3, area_threshold=3, merge=True, margin=50): #red blobs
       # print(blob.pixels())
        #if False:
        if blob.pixels() > 1000:
            xpos = 0
            ...
            #return [xpos,ypos]
    for blob in img.find_blobs(pink_tape_1, pixels_threshold=3, area_threshold=3, merge=True, margin=50): #red blobs
       # print(blob.pixels())
        #if False:
        if blob.pixels() > 1000:
            xpos = 0
            ...
            coord = [xpos,ypos]
            #img.draw_cross(xpos, ypos)
            img.draw_circle(blob.enclosing_circle(),color=(255,0,0))
            if coord not in list_blob:
                unique_list.append(coord)
            list_blob.append(coord)
            #return [xpos,ypos]
